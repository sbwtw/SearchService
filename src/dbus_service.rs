...
impl<M: MethodType<D>, D: DataType> MsgHandler for MessageHandler<M, D> {
    ...
    f.interface(SEARCH_CONTEXT_INTERFACE, service)
        .add_p(f.property::<String, _>("Context", ())
            ...
            .on_set(|i, m| {
                ...
                context.as_ref().map(move |x| x.write().unwrap().set_context(value));

                Ok(())
            })
            .on_get(|i, m| {
                let context: &Option<RwLock<SearchContext>> = m.path.get_data();
                i.append(context.as_ref().map(|x| x.read().unwrap().context().to_owned()).unwrap());

                Ok(())
            })
        )

        .add_m(f.method("Search", (), |m| {
